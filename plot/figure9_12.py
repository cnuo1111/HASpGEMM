...
import matplotlib.pyplot as plt
# a,b,c,d and e are the data to be modified
# which are the number of data obtained by SPA, HASH, ESC, HASpGEMM, and oneMKL, respectively
...
for cycle in range(a):
	...
	SPA[cycle] = (float)(content[cycle][6])
	
filename = './DATA/12-hash.csv'
b = 2628
with open(filename) as csvfile:
	reader = csv.reader(csvfile)
	rows = [row for row in reader]    
...
for cycle in range(e):
	matrix5[cycle] = math.log(abs((float)(content[cycle][2])),10)
	mkl[cycle] = (float)(content[cycle][4])
