...
#pragma omp parallel for
    ...
    Test_d_csrmultcsr(option, mat_name, nA, mA, nB, mB, csrRowPtrA_1based, csrColIdxA_1based, csrValA, csrRowPtrB_1based, csrColIdxB_1based, csrValB, 1, &nnzCub, &avg_nnzCub, &max_nnzCub, &min_nnzCub, &avg_time_esc, &min_time_esc, &avg_gflops_esc, &max_gflops_esc, &C_row_variation, &repeat_value);

    printf("ESC,%s,%d,%d,%.3lf,%.3lf,%.3lf\n",mat_name,nnzA,mA,avg_nnzCub,avg_gflops_esc,max_gflops_esc);
