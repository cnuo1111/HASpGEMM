...
#pragma omp parallel for
    ...
    Test_d_csrmultcsr(option, mat_name, nA, mA, nB, mB, csrRowPtrA_1based, csrColIdxA_1based, csrValA, csrRowPtrB_1based, csrColIdxB_1based, csrValB, 2, &nnzCub, &avg_nnzCub, &max_nnzCub, &min_nnzCub, &avg_time_hash, &min_time_hash, &avg_gflops_hash, &max_gflops_hash, &C_row_variation, &repeat_value);

    printf("HASH,%s,%d,%d,%.3lf,%.3lf,%.3lf\n",mat_name,nnzA,mA,avg_nnzCub,avg_gflops_hash,max_gflops_hash);
